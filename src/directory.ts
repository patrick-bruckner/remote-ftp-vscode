...
class Directory {
  ...
  exists(name: string, isdir: boolean) {
    if (isdir) {
  ...
  open(recursive: boolean, complete?) {
    ...
    client.list(this.remote, false, (err, list) => {
      ...
      this.root.emitter.emit('did-change-items', this);
    });
  }

  openPath(opath: string) {
  ...
  get local() {
    ...
    // return multipleHostsEnabled() === true ? this.client.projectPath :  vscode.workspace.workspaceFolders[0].name;
    return multipleHostsEnabled() === true ? this.client.projectPath : './';
  }
