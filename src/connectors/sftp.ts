...
class ConnectorSFTP extends Connector {
  ...
  connect(info, completed) {
    ...
    try {
      ...
      //   dismissable: true,
      // });
      console.log('SFTP connection attempt failed');
    }
          ...
          if (item.attrs.isSymbolicLink()) {
            ...
            self.sftp.realpath(fname, (realPatherr, target) => {
              if (realPatherr) {
                ...
                //   dismissable: false,
                // });
                console.log('Could not call realpath for symlink: ' + realPatherr);
              ...
              self.sftp.stat(target, (statErr, stats) => {
                if (statErr) {
                  ...
                  //   dismissable: false,
                  // });
                  console.log('Could not correctly resolve symlink: ' + `${fname} -> ${target}`);
  ...
  get(path, recursive, completed, progress?, symlinkPath?): any {
    const self = this;
  ...
  putDirect(file) {
    ...
    const stats = (err, attrs) => {
      ...
      function applyProgress() {
        if (typeof file.progress !== 'function') return;
      ...
      writeStream.on('finish', () => {
        ...
        if (!hasProp && (writeErr.message === 'No such file' || writeErr.message === 'NO_SUCH_FILE')) {
          self.mkdir(Path.dirname(fileObject.remotePath).replace(/\\/g, '/'), (dirErr) => {
            if (dirErr) {
  ...
  put(path, completed, progress?): any {
    const self = this;
  ...
  mkdir(path, completed, recursive?): any {
    const self = this;
