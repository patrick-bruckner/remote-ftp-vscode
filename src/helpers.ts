...
export const validateConfig = (client, data) => {
  try {
    ...
    //   dismissable: false,
    // });
    console.log('Could not parse `.ftpconfig`' + `${error.message}`);

    // open .ftpconfig file and mark the faulty line
    vscode.workspace.openTextDocument(client.getFilePath('.ftpconfig')).then((file) => {
      vscode.window.showTextDocument(file).then((editor) => {
        if (lineNumber === -1) return; // no line number to mark

        let dec = vscode.window.createTextEditorDecorationType({backgroundColor: "rgba(255, 0, 0, 0.5)",
                                                                isWholeLine: true});
        editor.setDecorations(dec, [new vscode.Range(lineNumber, 0, lineNumber, Infinity)]);
      });
    });
  }
...
export function statsToPermissions(stats) {
  ...
  return {
    other: Object.keys(PER_OTHER).map((elem: any) => ((stats.mode & elem) ? PER_OTHER[elem] : '')).reverse().join(''),
    group: Object.keys(PER_GROUP).map((elem: any) => ((stats.mode & elem) ? PER_GROUP[elem] : '')).reverse().join(''),
    user: Object.keys(PER_OWNER).map((elem: any) => ((stats.mode & elem) ? PER_OWNER[elem] : '')).reverse().join(''),
  };
}
