...
export default class Client {
    ...
    self.watch = {
      ...
      addListeners() {
        ...
        const watchUniq = (watchDatum) => {
          ...
          let watcher = vscode.workspace.createFileSystemWatcher(watchDatum, true, false, true);
          watcher.onDidChange((event: vscode.Uri) => {
            if (!event.path.match(/(^|[/\\])\../)) {
              self.watch.queueUpload.apply(self, [event.fsPath]);
            }
          });
  ...
  getFilePath(relativePath: string): string | boolean {
    const self = this;
  ...
  getProjectPath() {
    ...
    if (multipleHostsEnabled() === true) {
      ...
      const firstDirectory = vscode.workspace.workspaceFolders[0];
      if (firstDirectory != null) projectPath = firstDirectory.uri.fsPath;
    }
  ...
  updateIgnore() {
    ...
    if (Path.basename(self.ignorePath) === self.ignoreBaseName) {
      self.ignoreFilter = ignore().add(FS.readFileSync(self.ignorePath).toString('utf8'));
      return true;
    }
  ...
  doConnect() {
    ...
    //   dismissable: false,
    // });
    console.log('Remote FTP: Connecting...');
      ...
      case 'sftp': {
        ...
        if (self.info.privatekey) {
          ...
          try {
            ...
            //   dismissable: true,
            // });
            console.log('Remote FTP: Could not read privateKey file');
          }
        }
    ...
    self.connector.connect(info, () => {
      if (self.root.status !== 1) self.root.open(true);
      ...
      //   dismissable: false,
      // });
      console.log('Remote FTP: Connected');
    });
    ...
    self.connector.on('closed', (action) => {
      ...
      //   dismissable: false,
      // });
      console.log('Remote FTP: Connection closed');
    ...
    self.connector.on('error', (err, code) => {
      ...
      //   dismissable: false,
      // });
      console.log('Remote FTP: Connection failed');
    });

    // self.watchDebug(atom.config.get('remote-ftp.dev.debugResponse'));
    self.watchDebug(false);
  }

  watchDebug(isWatching: boolean) {
  ...
  toRemote(local: string) {
    const self = this;

    return Path.join(
      self.info.remote,
      vscode.workspace.asRelativePath(local, false),
    ).replace(/\\/g, '/');
  }

  toLocal(remote: string) {
    const self = this;
    const projectPath = self.getProjectPath();
    ...
    if (typeof remote !== 'string') {
      throw new Error(`Remote FTP: remote must be a string, was passed typeof remote` + (typeof remote));
    }
  ...
  list(remote: string, recursive: boolean, callback) {
    const self = this;
    self.onceConnected(() => {
      self._enqueue(() => {
  ...
  download(remote: string, recursive: boolean, callback) {
    const self = this;
  ...
  syncRemoteFileToLocal(remote: string, callback) {
    const self = this;
    ...
    if (self.status === 'CONNECTED') {
      ...
      //   dismissable: true,
      // });
      console.log('Remote FTP: Not connected!');
    }
    return self;
  }

  syncRemoteDirectoryToLocal(remote: string, isFile: boolean, callback) {
  ...
  syncLocalFileToRemote(local: string, callback) {
    const self = this;
  ...
  syncLocalDirectoryToRemote(local: string, callback) {
    const self = this;
        ...
        self.connector.list(remote, true, (err, remotes) => {
          ...
          traverseTree(local, (locals) => {
            ...
            const n = () => {
              ...
              if (!nRemote || remote.length !== nLocal.size) {
  ...
  mkdir(remote: string, recursive: boolean, callback) {
    const self = this;
    self.onceConnected(() => {
      self._enqueue(() => {
  ...
  rename(source: string, dest: string, callback) {
    const self = this;
    self.onceConnected(() => {
      self._enqueue(() => {
  ...
  chmod(path: string, mode, callback) {
    this.onceConnected(() => {
      this.connector.chmod(path, mode, callback);
    });
  }

  chown(path: string, uid, gid, callback) {
    this.onceConnected(() => {
  ...
  chgrp(path: string, uid, gid, callback) {
    this.onceConnected(() => {
      this.connector.chgrp(path, uid, gid, callback);
    });
  }

  openPasswordPrompt(isInteractive: boolean): Thenable<string|undefined> {
    let prompt = ''
    if (isInteractive) {
      prompt = 'Enter Vertification Code for keyboard-interactive';
    } else {
      prompt = 'Enter password/passphrase only for this session';
    }
    return vscode.window.showInputBox({prompt: prompt, password: true});
  }

  promptForPass() {
    const self = this;
    const dialog = this.openPasswordPrompt(false);
    dialog.then((pass) => {
      self.info.pass = pass;
  ...
  promptForKeyboardInteractive() {
    const self = this;
    const dialog = this.openPasswordPrompt(true);
